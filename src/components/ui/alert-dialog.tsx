...
export function AlertDialog({open,onOpenChange:_onOpenChange,children}:{open:boolean;onOpenChange:(v:boolean)=>void;children:React.ReactNode}){return <>{open?<div className="fixed inset-0 z-[80] grid place-items-center bg-bg-deep/70 p-4">{children}</div>:null}</>}
