...
export function useTestTelegram(){return useMutation({mutationFn:async(_ignoredChatId?:string):Promise<void>=>{const {error}=await supabase.functions.invoke('telegram-test',{body:{}});if(error)throw error;},onSuccess:()=>toast.success('Тестовое сообщение отправлено')});}
