...
export function useTestTelegram(){return useMutation({mutationFn:async(_ignoredChatId?:string):Promise<void>=>{const {data,error}=await supabase.functions.invoke<TelegramTestResponse>('telegram-test',{body:{}});if(error)throw error;if(!data?.ok)throw new Error('TELEGRAM_TEST_FAILED');},onSuccess:()=>toast.success('Тестовое сообщение отправлено')});}
